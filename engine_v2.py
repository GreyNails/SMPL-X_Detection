...
# Add this function to engine.py after the existing inference function

@torch.no_grad()
def inference_json(model,
                  criterion,
                  postprocessors,
                  data_loader,
                  device,
                  ...
                  logger=None):
    """
    Modified inference function for JSON dataset processing.
    Returns pose and camera data instead of saving visualizations.
    """
    try:
        need_tgt_for_training = args.use_dn
    except:
    ...
    for data_batch in metric_logger.log_every(
        ...
        # Call inference to extract pose data
        dataset.inference(result)
    
    # Get the extracted pose and camera data
    if hasattr(dataset, 'get_pose_data'):
        pose_data, camera_data = dataset.get_pose_data()
        return pose_data, camera_data
    else:
        return [], []